...
For background operation, use 'prismctl db start' instead.`,
...
Writes a PID file to ~/.productbuildershq/prism/server.pid and saves the
DSN to the config file. Use 'prismctl db stop' to shut it down.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			pid, _, err := readPIDFile()
			if err == nil && isProcessAlive(pid) {
				return fmt.Errorf("server already running (PID %d). Use 'prismctl db restart' to restart", pid)
			}
...
func dbStatusCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cmd.Printf("  PID:  %d\n", pid)
			cmd.Printf("  Port: %d\n", port)
			cmd.Printf("  DSN:  root:@tcp(127.0.0.1:%d)/prismcontrol\n", port)
			return nil
		},
	}
}
