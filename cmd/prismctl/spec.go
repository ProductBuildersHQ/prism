...
func specInitCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if init.HomeRepo == "" {
				return fmt.Errorf("initiative %s has no home-repo set; run 'prismctl initiative update %s --home-repo <repo-id>' first", init.ID, init.ID)
			}
...
func specJudgeShowCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cmd.Printf("prismctl spec judge record %s %s --score <0-10> --rationale \"<why>\" --model <your-model-id>\n", initID, specFile)
			return nil
		},
	}
}
