...
import (
	...
	"github.com/ProductBuildersHQ/prism-control/pkg/roadmap"
...
func checkRoadmapDrift(cmd *cobra.Command, svc *service.Service, initiativeID string) {
	...
	for i, d := range diffs {
		if i >= limit {
			cmd.Printf("  ... and %d more (run: prismctl roadmap diff %s)\n", len(diffs)-limit, roadmapPath)
			break
		}
			...
			if transition {
				for initID := range initiativesSeen {
					if svc.CheckInitiativeAllComplete(cmd.Context(), initID) {
						...
						cmd.Printf("    prismctl initiative transition %s delivery_complete\n", initID)
					}
				}
			}
			return nil
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(result.Completed) == 0 {
				...
				if len(result.NoAssignment) > 0 {
					...
					cmd.Println("  These RMIs may have expired leases — reclaim with: prismctl work claim <RMI-ID> --worker <session-id>")
				}
				return nil
			}
