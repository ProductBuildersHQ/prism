...
func configCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "config",
		Short: "Manage prismctl configuration",
		Long: `View and modify the prismctl configuration file.

Configuration is stored at ~/.productbuildershq/prism/config.json.
...
func configShowCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			} else if env := os.Getenv("PRISMCTL_DSN"); env != "" {
				cmd.Printf("  %s  (from $PRISMCTL_DSN)\n", env)
			} else if cfg.DSN != "" {
